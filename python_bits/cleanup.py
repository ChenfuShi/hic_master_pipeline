...
def cleanup(Configuration, obj = "normal"):
    ...
    if obj == "normal":
        ...
        proc = subprocess.run("find -L ./ -maxdepth 6 -type f ! -name '*.gz' ! -name '*.bam' -print0 | xargs -L1 -P8 -0 gzip",shell=True, cwd = matrix_loc)
        proc = subprocess.run("find -L ./ -maxdepth 6 -type f ! -name '*.gz' ! -name '*.bam' -print0 | xargs -L1 -P8 -0 gzip",shell=True, cwd = data_loc)
